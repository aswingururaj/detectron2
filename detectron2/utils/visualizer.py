...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        if predictions.has("pred_masks"):
            ...
            masks.append(np.full((self.output.height, self.output.width),True))
        else:
            masks = None
        ...
        for i in range(len(labels)):
            if 'cup' in labels[i]:
                for k in range(num):
                    test = np.where(masks[i]._mask==1,255,0)
                    test = np.uint8(test)
                    edges = cv2.Canny(test,100,200)
                    annotations = np.argwhere(edges == 255)
                    masks[i]._mask[annotations[:,0],annotations[:,1]] = 0
            if 'bowl' in labels[i]:    
                for k in range(num_bowl):
                    test = np.where(masks[i]._mask==1,255,0)
                    test = np.uint8(test)
                    edges = cv2.Canny(test,100,200)
                    annotations = np.argwhere(edges == 255)
                    masks[i]._mask[annotations[:,0],annotations[:,1]] = 0
            if 'suitcase' in labels[i]:    
                for k in range(num_bowl):
                    test = np.where(masks[i]._mask==1,255,0)
                    test = np.uint8(test)
                    edges = cv2.Canny(test,100,200)
